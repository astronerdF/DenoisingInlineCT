...
from noise2noise.unet import UNet
from noise2noise.utils import *
...
class Noise2Noise(object):
    ...
    def eval(self, valid_loader):
        ...
        for batch_idx, (source, target) in enumerate(valid_loader):
            ...
            # Use the actual size of the batch instead of assuming the batch size is consistent
            batch_size = source.size(0)  # Get the actual batch size for this batch

            for i in range(batch_size):
                psnr_value = psnr(source_denoised[i].cpu(), target[i].cpu()).item()
